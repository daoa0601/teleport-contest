...
| C and Lua level construction | partial | js/mklev.js<br/>js/weight.js<br/>js/dungeon.js<br/>js/lua_des.js<br/>js/regions.js<br/>js/monmove.js<br/>js/light.js<br/>js/vision.js<br/>js/allmain.js | fastforward.mineralize<br/>seeded room-fill compatibility paths | The ordinary-room orchestration and exact 31-name themed-room dispatch now have direct witnesses; Ghost, Cloud, Boulder, Spider, Garden, Trap, Massacre, and Statuary fills are live source callbacks. Buried treasure lacks the shared ROT_ORGANIC dispatcher; Buried zombies owns depth-sensitive construction plus a focused source-turn zombify/revival/pit edge, but not the generic cross-type timer queue or complete fill_pit and failure/message variants; Light source owns oil-lamp construction, mobile illumination, fuel breakpoints, and expiration but not warning text or the generic timer queue. Exact source ordering across all shapes, seven incomplete fill callbacks, many special-level operations, legacy seeded generation branches, incomplete monster/trap/special-object constructors, and a sealed gate remain open. |
...
| C and Lua level construction | Fill: Ice room | absent | Terrain conversion and per-square melt timers are not ported. |
...
| C and Lua level construction | Fill: Buried treasure | partial | Initialized-then-cleared chest contents, source-ordered burial draws, contained child objects and weight, retained coordinates, and the deferred directional burn engraving have a direct zero-bridge witness. The ROT_ORGANIC deadline is recorded, but the shared timer queue and rot_organic dispatch remain absent. |
| C and Lua level construction | Fill: Buried zombies | partial | Depth-sensitive eligibility, per-corpse shuffle and dry placement, initialized corpse replacement, burial, ROT_CORPSE cancellation, absolute ordered ZOMBIFY_MON deadlines, focused source-turn dispatch, living-to-zombie replacement, gender-preserving NO_MINVENT revival, successful corpse removal, and pit creation have direct zero-bridge witnesses. The generic cross-type timer-id queue, fill_pit boulder effects, blocked and genocided regression carriers, exhaustive adjacent relocation, and every visibility/audio continuation remain open. |
...
| C and Lua level construction | Fill: Light source | partial | The initialized lit oil lamp, source fuel breakpoints, saveable deadline state, global-turn expiration, and radius-three mobile illumination have a direct zero-bridge witness. Threshold warning text, other burning object types, timer-id ordering, and the generic timer queue remain open. |
