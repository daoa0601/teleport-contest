...
| C and Lua level construction | Fill: Buried zombies | partial | Depth-sensitive living-species eligibility, per-corpse shuffle and dry placement, initialized corpse replacement, burial draw and chain state, ROT_CORPSE cancellation, and absolute ZOMBIFY_MON deadlines have a direct zero-bridge witness. The shared monster constructor now honors NO_MINVENT without skipping non-inventory birth state. The generic timer queue, zombie_form dispatch, occupied-square relocation and revival invocation, pit emergence, messages, and corpse removal remain absent. |
