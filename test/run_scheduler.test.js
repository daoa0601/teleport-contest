...
test('seed0011 iron-golem polyself rehumanizes on rust-monster touch',
    async () => {
        const result = await runSegment({
            seed: 11,
            datetime: '20000110090000',
        ...
        assertRngSliceExact(result.getRngSlices()[24], [
            'rn2(2)=0', 'rn2(19)=13', 'rn2(10)=5',
            'rn2(500)=200',
        ], 'seed0011 fixed iron-golem form setup RNG');
        assert.equal(decodedTopline(result.getScreens()[24]),
            'You turn into an iron golem!  The clasp on your cloak breaks open!--More--');
        assert.equal(decodedRow(result.getScreens()[24], 23),
            'Dlvl:1 $:0 HP:120(120) Pw:7(7) AC:3 HD:18');
        assert.deepEqual(result.getCursors()[24], [74, 0, 1]);

        assertRngSliceExact(result.getRngSlices()[25], [],
            'seed0011 iron-golem ability notice RNG');
        assert.equal(decodedTopline(result.getScreens()[25]),
            'Use the command #monster to use your breath weapon.');

        assertRngSliceExact(result.getRngSlices()[53], [
            'rn2(12)=2', 'rn2(12)=2', 'rn2(70)=8',
            'rn2(300)=32', 'rn2(200)=70', 'rn2(20)=12',
            'rn2(85)=1', 'rn2(5)=2', 'rnd(20)=12', 'd(0,0)=0',
            'rn2(3)=1', 'rn2(6)=1', 'rnd(21)=2', 'd(0,0)=0',
        ], 'seed0011 iron-golem complete-rust rehumanization RNG');
        ...
        assert.equal(decodedRow(result.getScreens()[53], 23),
            'Dlvl:1 $:0 HP:12(12) Pw:7(7) AC:10 Xp:1');
        assert.deepEqual(result.getCursors()[53], [75, 0, 1]);
...
test('seed0011 gelatinous-cube nohands pushes off boots after load pager',
    async () => {
        const result = await runSegment({
            seed: 11,
            datetime: '20000110090000',
        ...
        assert.equal(result.getScreens().length, 72);
        assertRngSliceExact(result.getRngSlices()[63], [
            'rn2(2)=1', 'rn2(19)=13', 'rn2(500)=327', 'd(6,8)=25',
        ], 'seed0011 gelatinous-cube boot setup RNG');
        assert.equal(decodedTopline(result.getScreens()[63]),
            'You turn into a gelatinous cube!  You drop your gloves and weapon!--More--');
        assert.equal(decodedRow(result.getScreens()[63], 23),
            'Dlvl:1 $:1540 HP:25(25) Pw:5(5) AC:5 HD:6 Burdened Blind');
        assert.deepEqual(result.getCursors()[63], [74, 0, 1]);

        assertRngSliceExact(result.getRngSlices()[64], [],
            'seed0011 gelatinous-cube boot capacity RNG');
        assert.equal(decodedTopline(result.getScreens()[64]),
            'Your movements are slowed slightly because of your load.--More--');
        assert.deepEqual(result.getCursors()[64], [64, 0, 1]);
        assert.equal(decodedTopline(result.getScreens()[65]),
            'Your boots are pushed off your feet!');
        assert.equal(decodedRow(result.getScreens()[65], 23),
            'Dlvl:1 $:1540 HP:25(25) Pw:5(5) AC:8 HD:6 Burdened Blind');
        assert.deepEqual(result.getCursors()[65], [65, 6, 1]);
...
test('seed0011 acid-blob very-small form slides boots off',
    async () => {
        const result = await runSegment({
            seed: 11,
            datetime: '20000110090000',
        ...
        assert.equal(result.getScreens().length, 66);
        assertRngSliceExact(result.getRngSlices()[57], [
            'rn2(2)=1', 'rn2(19)=13', 'rn2(500)=327', 'd(1,8)=2',
        ], 'seed0011 acid-blob boot setup RNG');
        assert.equal(decodedTopline(result.getScreens()[57]),
            'You turn into an acid blob!  You drop your gloves and weapon!--More--');
        assert.equal(decodedRow(result.getScreens()[57], 23),
            'Dlvl:1 $:1540 HP:2(2) Pw:5(5) AC:5 HD:1 Overloaded Blind');
        assert.deepEqual(result.getCursors()[57], [69, 0, 1]);

        assertRngSliceExact(result.getRngSlices()[58], [],
            'seed0011 acid-blob overloaded notice RNG');
        assert.equal(decodedTopline(result.getScreens()[58]),
            "You can't even move a handspan with this load!--More--");
        assert.deepEqual(result.getCursors()[58], [54, 0, 1]);
        assert.equal(decodedTopline(result.getScreens()[59]),
            'Your boots slide off your feet!');
        assert.equal(decodedRow(result.getScreens()[59], 23),
            'Dlvl:1 $:1540 HP:2(2) Pw:5(5) AC:8 HD:1 Overloaded Blind');
        assert.deepEqual(result.getCursors()[59], [65, 6, 1]);
        ...
        assert.ok(floorObjects.some(object => object.otyp === 39));
        assert.ok(floorObjects.some(object => object.otyp === 163));
        assert.equal(game.u.umonnum, 6);
        assert.equal(game.u.mh, 2);
        assert.equal(game.u.uac, 8);
        assert.equal(game.u._encumbrance, 'Overloaded');
        assert.equal(game.blind, true);
        assert.equal(game.context.move, 0);
    });
...
test('seed0014 Warning-bearing Hallucination rebuild preserves display phase',
    async () => {
        const moves = '  n#levelchange\n30\n' + ' '.repeat(40)
            + '#wizgenesis\nhostile Wizard of Yendor\ny'
            + '#wizintrinsic\nh\n '
        ...
        assertRngSliceExact(result.getRngSlices()[113], [],
            'seed0014 Warning Hallucination activation RNG');
        assert.equal(decodedTopline(result.getScreens()[113]),
            'Oh wow!  Everything looks so cosmic!--More--');
        assert.equal(decodedRow(result.getScreens()[113], 23),
            'Dlvl:1 $:1172 HP:144(144) Pw:288(288) AC:8 Xp:30 Hallu');
        assert.deepEqual(result.getCursors()[113], [44, 0, 1]);

        assertRngSliceExact(result.getRngSlices()[114], [],
            'seed0014 Warning docrt acknowledgement RNG');
        const warningRedraw = decodeScreen(result.getScreens()[114]);
        assert.deepEqual(warningRedraw[3][12], {
            ch: 'D', color: 14, attr: 0, decgfx: 0,
        });
        assert.deepEqual(result.getCursors()[114], [11, 3, 1]);

        assert.equal(result.getRngSlices()[122].length, 60);
        assert.equal(decodedTopline(result.getScreens()[122]),
            'The pit fiend hits!  The wire shark casts a spell!  Double Trouble...--More--');
        assert.deepEqual(result.getCursors()[122], [77, 0, 1]);
...
test('seed0052 inactive fog hook stays between active actors',
    async () => {
        const fullMoves = '  n#levelchange\n30\n' + ' '.repeat(40)
            + '#wizgenesis\nhostile Wizard of Yendor\ny'
            + '#wizwish\nwand of death\nzkh   '
        ...
        const result = await runSegment({
            ...
            datetime: '20000110090000',
            nethackrc: HALLUCINATED_DEATH_TOUCH_RC,
            moves: fullMoves.slice(0, 572),
            storage: new Map(),
        });

        assert.equal(result.getScreens().length, 573);
        assertRngSliceExact(result.getRngSlices()[572], [
            'rn2(5)=4', 'rn2(32)=17', 'rn2(5)=1',
            'rn2(3)=2', 'rn2(5)=3', 'rn2(8)=7',
            'rn2(5)=4', 'rn2(5)=4', 'rnd(20)=17',
            'd(3,5)=9', 'rnd(8)=7', 'rnd(4)=3',
            'rn2(3)=1', 'rn2(6)=0', 'rn2(5)=2',
            'rn2(24)=12', 'rn2(28)=25', 'rn2(32)=14',
            'rn2(20)=17', 'rn2(5)=4', 'rn2(12)=7',
            'rn2(12)=4', 'rn2(12)=4', 'rn2(12)=4',
            'rn2(12)=8', 'rn2(12)=11', 'rn2(12)=1',
            'rn2(12)=9', 'rn2(25)=15', 'rn2(100)=28',
            'rn2(400)=25', 'rn2(20)=2', 'rn2(67)=39',
        ], 'seed0052 fmon-interleaved fog hook RNG');
        ...
        const fog = game.level.monsters.find(monster =>
            monster.m_id === 91);
        assert.ok(fog);
        assert.deepEqual([fog.mx, fog.my], [24, 9]);
        assert.equal(fog.movement, 0);
        assert.deepEqual(game.level.regions.map(region => ({
            ttl: region.ttl,
            cells: region.cells.map(cell => [cell.x, cell.y]),
...
test('seed0011 high-cleric mace bonus closes the complete fire session',
    async () => {
        const result = await runSegment({
            seed: 11,
            datetime: '20000110090000',
        ...
        assertRngSliceExact(result.getRngSlices()[111], [
            'rnd(20)=19', 'd(4,10)=25', 'rnd(6)=5',
        ], 'seed0011 high-cleric mace attack prefix RNG');
        assert.equal(decodedTopline(result.getScreens()[111]),
            'The renegade priest of Poseidon swings his mace.--More--');
        assert.equal(decodedRow(result.getScreens()[111], 23),
            'Dlvl:1 $:1540 HP:104(141) Pw:254(254) AC:8 Xp:30');

        assertRngSliceExact(result.getRngSlices()[114], [
            'rn2(3)=2', 'rn2(6)=4', 'rnd(21)=12', 'd(2,8)=9',
        ], 'seed0011 mace damage tail and next kick RNG');
        assert.equal(decodedTopline(result.getScreens()[114]),
            'The renegade priest of Poseidon hits!--More--');
        assert.equal(decodedRow(result.getScreens()[114], 23),
            'Dlvl:1 $:1540 HP:70(141) Pw:254(254) AC:8 Xp:30');

        assertRngSliceExact(result.getRngSlices()[115], [
            'rn2(3)=2', 'rn2(6)=4', 'rn2(25)=22',
            'rn2(13)=0', 'rn2(5)=3', 'rn2(5)=0', 'rn2(25)=11',
        ], 'seed0011 high-cleric kick tail and dual spell selection RNG');
        assert.equal(decodedTopline(result.getScreens()[115]),
            'The renegade priest of Poseidon kicks!--More--');
        assert.equal(decodedRow(result.getScreens()[115], 23),
            'Dlvl:1 $:1540 HP:61(141) Pw:254(254) AC:8 Xp:30');

        assertRngSliceExact(result.getRngSlices()[116], [
            'rn2(25)=24', 'rn2(13)=5', 'rn2(13)=11',
        ], 'seed0011 second high-cleric blocked spell RNG');
        assert.equal(decodedTopline(result.getScreens()[116]),
            'The renegade priest of Poseidon points at you, then curses.--More--');
        assert.equal(decodedRow(result.getScreens()[117], 23),
            'Dlvl:1 $:1540 HP:61(141) Pw:254(254) AC:9 Xp:30');

        assert.equal(game.uarmg?.oeroded, 1);
        assert.equal(game.u.uac, 9);
        assert.equal(game.u.uhp, 61);
        const cleric = game.level.monsters.find(monster =>
            monster.mnum === 276);
        assert.ok(cleric);
        assert.equal(cleric.mw?.otyp, 73);
        assert.equal(cleric.mw?.spe, 3);
        assert.equal(game.context.move, 0);
    });
...
test('seed0025 timed antimagic scales curse-items and preserves mutations',
    async () => {
        const result = await runSegment({
            seed: 25,
            datetime: '20000110090000',
        ...
        assert.equal(result.getScreens().length, 140);
        assert.equal(decodedRow(result.getScreens()[74], 17),
            ' q - magic resistance');
        ...
        assertRngSliceExact(result.getRngSlices()[121], [
            'd(14,8)=70',
        ], 'seed0025 antimagic curse-items cast pre-roll RNG');
        assert.equal(decodedTopline(result.getScreens()[121]),
            'The renegade priest of Poseidon casts a spell at you!--More--');

        assertRngSliceExact(result.getRngSlices()[122], [],
            'seed0025 antimagic need-help pager RNG');
        assert.equal(decodedTopline(result.getScreens()[122]),
            'You feel as if you need some help.--More--');
        ...
        assertRngSliceExact(result.getRngSlices()[123], [
            'rnd(3)=3', 'rnd(11)=2', 'rnd(11)=1',
            'rnd(11)=7', 'rn2(25)=8',
        ], 'seed0025 antimagic scaled rndcurse and continuation RNG');
        assert.equal(decodedTopline(result.getScreens()[123]),
            'You feel a malignant aura surround you.--More--');
        assert.equal(decodedRow(result.getScreens()[123], 23),
            'Dlvl:1 $:1032 HP:131(137) Pw:273(273) AC:8 Xp:30');
